...
int main() {
  ...
  config_type monitor_config = reelay::json::parse(R"( {
    ...
    "pattern": "historically[0:5]{timestamp, bruh}"
    })");
...
std::int64_t getFieldValueAsString(const Message* msg, const std::string& fieldName) {
  ...
  // You need to choose the right accessor based on the field type:
  if (field->cpp_type() == FieldDescriptor::CPPTYPE_MESSAGE) {
    const Message& bruh = reflection->GetMessage(*msg, field);
    return bruh.GetReflection()->GetInt64(bruh, bruh.GetDescriptor()->FindFieldByName("seconds"));
  }
...
void data_handler(const zenoh::Sample &sample) {
    // osi3::GroundTruth gt;
    // gt.ParseFromArray(sample.get_payload().as_string().c_str(), sample.get_payload().size());
    // gt.GetDescriptor()->FindFieldByLowercaseName("timestamp");
    std::cout << "bruh-1" << std::endl;
    const Descriptor* descriptor = DescriptorPool::generated_pool()->FindMessageTypeByName("osi3.ground_truth");
    std::cout << descriptor->name() << std::endl;
    const Message* prototype = MessageFactory::generated_factory()->GetPrototype(descriptor);
    std::cout << "bruh" << std::endl;
    Message* gt = prototype->New();
    std::cout << "bruh2" << std::endl;
    gt->ParseFromArray(sample.get_payload().as_string().c_str(), sample.get_payload().size());
    std::cout << getFieldValueAsString(gt, "timestamp") << std::endl;

    // auto result = generated_monitor->update(gt);
...
// osi3::GroundTruth gt;
// std::unordered_map <std::string, reelay::monitor<input_type, output_type>* > monitors;
