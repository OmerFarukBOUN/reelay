...
//
#include "reelay/pub_sub_connection/parser/ptl_grammar.hpp"
...
struct ptl_parser : ptl_grammar {
  ...
  explicit ptl_parser(const reelay::kwargs& mm = reelay::kwargs()) : meta(mm)
  {
    ...
    parser["URI"] = [&](const peg::SemanticValues& sv) {
      static int global_token_no = 0;
      std::cout << "URI" << std::endl;
      auto path = std::vector<path_token>();
      for(std::size_t i = 0; i < sv.size(); i++) {
        path.push_back(reelay::any_cast<path_token>(sv[i]));
      }
      global_path2token_map.try_emplace(
        path, std::to_string(global_token_no++));
      return global_path2token_map[path];
    };
  ...
  std::shared_ptr<network_t> make_shared(
    const std::string& pattern, const options_t& options = options_t())
  {
    node_ptr_t root;
    parser.parse(pattern.c_str(), root);
